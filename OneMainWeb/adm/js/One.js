...
function setUpHtmlEditing(enableHtml, enableCk, html) {
    ...
    if (enableCk === true) {
        ...
        $(".ckbox").show();
        ClassicEditor
            .create(document.querySelector('#content-html'), ckEditor5Options)
            .then(editor => { contentHtmlEditor = editor })
    } else {
        if (enableHtml === true) {
...
function buildFormControl(fieldLabel, fieldId, fieldType) {
    ...
    if (fieldType != 'builtin') {
        ...
        if (fieldType == 'html')
            controlHtml += '<textarea  class="form-control ckeditor5" id="' + fieldId + '" name="' + fieldId + '"></textarea>';
...
$(document).ready(function () {
    ...
    ClassicEditor.create(document.querySelector('.ckeditor5'), ckEditor5Options)
        .then(editor => {
            // console.log("ClassicEditor1");
    ...
    $('#text-content-modal a.btn-success').on('click', function (e) {
        ...
        if (contentHtmlEditor !== undefined) {
            content['Html'] = contentHtmlEditor.getData();
...
function populateForeignKeyOptions(virtualTableId, v, whenSelected) {
    $.ajax({
        ...
        data: {
            ...
            columnId: v.Id,
            limit: SUGGEST_ENTRIES_IN_DROPDOWN_LIMIT
        },
        dataType: "json",
        type: "GET",
        contentType: "application/json; charset=utf-8",
